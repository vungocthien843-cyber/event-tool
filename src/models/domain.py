...
class Service(Base):
    ...
    source_commit_ts: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    ...
    members: Mapped[list["ServiceMember"]] = relationship(back_populates="service", cascade="all, delete-orphan")
    dependencies: Mapped[list["ServiceDependency"]] = relationship(back_populates="service", cascade="all, delete-orphan")

    __table_args__ = (UniqueConstraint("repo_full_name", "file_path", name="uq_services_repo_file_path"),)
...
class ServiceMember(Base):
    ...
    service_key: Mapped[str] = mapped_column(ForeignKey("services.service_key", ondelete="CASCADE"))
